...
async function getV3Quote(
  ...
  if (!publicClient || amountIn === 0n || tokenIn.toLowerCase() === tokenOut.toLowerCase()) return null;
  ...
  return {
    ...best,
    feePct: best.fee === 500 ? "0.05%" : best.fee === 3000 ? "0.30%" : "1.00%",
  };
}
...
function SwapInner() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const result = await getV3Quote(publicClient, tokenIn.address, tokenOut.address, amtIn);
